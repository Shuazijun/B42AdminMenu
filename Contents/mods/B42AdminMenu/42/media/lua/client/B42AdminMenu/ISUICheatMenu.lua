...
require "B42AdminMenu/ItemSpawner/crucibleCore"
require "B42AdminMenu/Teleport/compassCore"
require "B42AdminMenu/Teleport/GPSWindow"
require "B42AdminMenu/Teleport/TeleportWindow"
require "B42AdminMenu/Vehicle/VehicleControlBox"
require "B42AdminMenu/Vehicle/VehicleSpawnCore"
require "B42AdminMenu/Lua/LuaInterpreter"
require "B42AdminMenu/Input/GenericInputWindow"
require "B42AdminMenu/Input/SkillInputWindow"

---@class ISUICheatMenu
---@field _index ISUICheatMenu
---@field ReloadedCount number
...
function ISUICheatMenu.CurrentObjects(worldobjects, subMenuObject)
	...
	for _,obj in ipairs(worldobjects) do
		if instanceof(obj, "IsoDoor") or (instanceof(obj, "IsoThumpable") and obj:isDoor()) then
			...
			if doorkey == -1 then
			   local NoKeyRequired = subMenuObject:addOption(getText("UI_CMRB_Object_NoKeyRequired"), worldobjects, nil)
	...
	if NoDoor == 1 and NoWindow == 1 then
	   local NoObjectFound = subMenuObject:addOption(getText("UI_CMRB_Object_NoObjectFound"), worldobjects, nil)
		...
		for k,v in pairs(CheatCoreCM.Parts) do
			if k ~= "nodisplay" then
				...
				--print(v)
					...
					subMenuMod:addOption(getText("UI_CMRB_Utility_VehicleParts_Repair"), worldobjects,
					function()
						v:setCondition(100)
					end)
					subMenuMod:addOption(getText("UI_CMRB_Utility_VehicleParts_Durability"), worldobjects,
					function()
						inputUI:setVisible(true); inputUI.func = function(val)
							v:setCondition(tonumber(val))
						end
					end)
					...
					if v:isContainer() == true then
						subMenuMod:addOption(getText("UI_CMRB_Utility_VehicleParts_SetResource"), worldobjects,
						function()
							inputUI:setVisible(true);
							inputUI.func = function(val)
								v:setContainerContentAmount(tonumber(val), false, false)
							end
						end)
					elseif k == "Engine" then
						subMenuMod:addOption(getText("UI_CMRB_Utility_VehicleParts_Quality"), worldobjects,
						function()
							inputUI:setVisible(true)
							inputUI.func = function(val)
								local num = tonumber(val)
								if num then
									car:setEngineFeature(num, car:getEngineLoudness(), car:getEnginePower())
								end
							end
						end)
						subMenuMod:addOption(getText("UI_CMRB_Utility_VehicleParts_Loudness"), worldobjects,
						function()
							inputUI:setVisible(true)
							inputUI.func = function(val)
								local num = tonumber(val)
								if num then
									car:setEngineFeature(car:getEngineQuality(), num, car:getEnginePower())
								end
							end
						end)
						subMenuMod:addOption(getText("UI_CMRB_Utility_VehicleParts_Power"), worldobjects,
						function()
							inputUI:setVisible(true)
							inputUI.func = function(val)
								local num = tonumber(val)
								if num then
									car:setEngineFeature(car:getEngineQuality(), car:getEngineLoudness(), num)
								end
							end
						end)
					...
					subMenuMod:addOption(getText("UI_CMRB_Utility_VehicleParts_Uninstall"), worldobjects, function() v:setInventoryItem(nil); car:update() end)
...
function ISUICheatMenu.createMenuEntries(_player, _context, _worldObjects)
	...
	ISUICheatMenu.worldobjects = worldobjects
...
function ISUICheatMenu:CharacterCheatMenu()
	...
	local God = subMenuCharacter:addOption(getText("UI_CMRB_Character_God"), worldobjects, function()
		local pdata = getPlayer():getModData()
		pdata.IsGod = not pdata.IsGod
		if pdata.IsGod then getPlayer():Say(getText("UI_CMRB_Character_God")..getText("UI_CMRB_Message_IsEnabled")) else getPlayer():Say(getText("UI_CMRB_Character_God")..getText("UI_CMRB_Message_IsDisabled")) end
		CheatCoreCM.dragDownDisable()
	end, nil );

	if getPlayer():getModData().IsGod then
		God.checkMark = true
	else
		God.checkMark = false
	end
...
--[[
	local InfiniteCarryWeight = subMenuMoodAndXP:addOption(getText("UI_CMRB_MoodAndXP_InfiniteCarryweight"), worldobjects, function() 
		CheatCoreCM.HandleToggle(getText("UI_CMRB_MoodAndXP_InfiniteCarryweight"), "CheatCoreCM.IsInfiniteCarryWeight") 
		CheatCoreCM.DoCarryWeightCheat() 
	end);

	InfiniteCarryWeight.checkMark = CheatCoreCM.IsInfiniteCarryWeight
]]
		...
		if ISUICheatMenu.AdminCheck() then
			...
			elseif not ISUICheatMenu.ModIntegrityCheck() and CheatCoreCM.DoLuaChecksum then
				if ISUICheatMenu.WarningMessageTrigger == 0 then
					local WaringBox = ISModalDialog:new(getCore():getScreenWidth() / 2 - 250, getCore():getScreenHeight() / 2 - 75, 500, 150, getText("UI_CMRB_Warning_ServerSetting"), false, nil, nil)
					WaringBox:initialise()
					WaringBox:setAlwaysOnTop(true)
					WaringBox:bringToTop()
