---@class VehicleSpawnDeploy : ISUIElement
---@field deploy string
---@field deploy1 string
---@field made boolean
---@field createBtn ISButton
...
function VehicleSpawnDeploy:createVehicleSpawn()
	...
	if isClient() then
		...
		local vehicle = addVehicleDebug(tostring(self.deploy1), player:getDir(), VehicleStatus, getCell():getGridSquare(x, y, z))
		if string.match(tostring(self.deploy1), "Smash") or string.match(tostring(self.deploy1), "Burnt") or string.match(tostring(self.deploy1), "wreck") then
...
function VehicleSpawnDeploy:new(x, y, width, height, deploy, deploy1, customID)
	local o = ISUIElement:new(x, y, width, height) ---@diagnostic disable-line: assign-type-mismatch
	setmetatable(o, self);
	self.__index = self;
	---@type VehicleSpawnDeploy
